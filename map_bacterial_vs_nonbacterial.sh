...
for BINNER in "${BINNERS[@]}"; do
    ...
    if [ -f "$BINNER_BACT_STATS" ] && [ -f "$BINNER_NONBACT_STATS" ]; then
        ...
        # Extract existing results from stats files (sum R1 + R2 mapped reads)
        bact_mapped=$(grep "^mapped:" "$BINNER_BACT_STATS" | \
            awk '{gsub(/,/,"",$3); sum+=$3} END {print sum+0}')
        bact_mapped=${bact_mapped:-0}

        nonbact_mapped=$(grep "^mapped:" "$BINNER_NONBACT_STATS" | \
            awk '{gsub(/,/,"",$3); sum+=$3} END {print sum+0}')
        nonbact_mapped=${nonbact_mapped:-0}

        # Get total reads if not set yet
        if [ $total_reads -eq 0 ]; then
            total_reads=$(grep "^Reads Used:" "$BINNER_BACT_STATS" | awk '{gsub(/,/,"",$3); print $3}' | tr -d '\n\r')
    ...
    if [ $total_reads -eq 0 ]; then
        if [ "$bact_seqs" -gt 0 ]; then
            total_reads=$(grep "^Reads Used:" "${TREATMENT_DIR}/${BINNER}_bacterial_stats.txt" | \
                awk '{gsub(/,/,"",$3); print $3}' | tr -d '\n\r')
        elif [ "$nonbact_seqs" -gt 0 ]; then
            total_reads=$(grep "^Reads Used:" "${TREATMENT_DIR}/${BINNER}_nonbacterial_stats.txt" | \
                awk '{gsub(/,/,"",$3); print $3}' | tr -d '\n\r')
        fi
        total_reads=${total_reads:-0}
    fi
